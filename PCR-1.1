...
unsigned time1, time2, time3; // 变性，退火，延伸所需要的时间 
...
unsigned int cycle_index; // 程序设定的循环次数
unsigned int cycle = 1; // 储存程序已经运行的次数
...
const byte rowPins[ROWS] = { 2,3,4,5 }; // 引脚号码
const byte colPins[COLS] = { 6,7,8,9 }; // 引脚号码

Keypad customKeypad = Keypad(makeKeymap(keys), rowPins, colPins, ROWS, COLS); // 初始化键盘

void update_input_display(int i); // 输入时函数原型
void update_display(); // 显示函数原型
void keep_temp(int tpmin, int tpmax, int time); // 温度控制函数原型
...
void loop()
{
  ...
  if (cycle <= cycle_index)
  {
    ...
    keep_temp(tp1min, tp1max, time1); // 进行温度操作

    /*************************退火***************************/
    steptemp = 4; // 储存当前步骤
    keep_temp(tp2min, tp2max, time2); // 进行温度操作

    /**************************退火**************************/
    steptemp = 5; // 储存当前步骤
    keep_temp(tp3min, tp3max, time3); // 进行温度操作
...
void update_input_display(int i)
{
...
void keep_temp(int tpmin, int tpmax, int time)
{
